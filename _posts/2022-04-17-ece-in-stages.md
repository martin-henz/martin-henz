...
The [next evaluator](https://share.sourceacademy.org/4h7wn) adds blocks, block-scoped `const` declarations,
...
``` js
            ...
            components = pair(body, 
                              pair(make_exit_scope_instruction(),
                                   continuation));
	} else ...
```
Between the body and the continuation, an exit-scope instruction indicates
to the evaluator that the validity of the extended environment has expired.
``` js
function make_exit_scope_instruction() {
    return list("exit_scope_instruction");
}
function is_exit_scope_instruction(instr) {
    return is_tagged_list(instr, "exit_scope_instruction");
}
```
The evaluation of an exit-scope instruction establishes the environment
before it was extended when the scope was entered.
``` js
        } else if (is_exit_scope_instruction(component)) {
            components = continuation;
